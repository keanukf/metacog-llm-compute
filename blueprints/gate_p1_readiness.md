...
- [ ] **HART — Beide Manifeste final und im Freeze-Tag.** `difficulty_manifest.json` für TextWorld und ToH mit 50 Instanzen, `difficulty_tier`, `holdout` (5/45); nach diesem Punkt unveränderlich. Ein Smoke bestätigt, dass `holdout`/`difficulty_tier` im Episode-JSON ankommen (Gate B belegt den Schreibpfad bereits). **Stand 2026-07-19:** beide Manifeste existieren und sind committed (TextWorld: `r5_i1_take+cook`, Seed 20260718; ToH: 4 Disks, `random_scramble`, Seed 271828), Mock-Smoke für beide bestanden. Noch offen: der eigentliche **Freeze-Tag** selbst (git tag) — bewusst noch nicht gesetzt, das ist eine Gate-Abschluss-Entscheidung, keine technische Restarbeit. *Evidenz: Manifeste im getaggten Commit.*
...
- [ ] **WEICH — H3-Power-Simulation.** §5.8 sieht eine simulationsbasierte Power-Prüfung für die Interaktion vor, geseedet mit Pilot-ICC und Entropieverteilung. Nach C-5 entweder durchführen oder die Limitation aktiv wählen und in §5.9 belassen. Vor Phase 1 entscheiden, welcher Pfad gilt. **Durchgeführt (2026-07-17):** echte Monte-Carlo-Power-Simulation (`scripts/h3_power_simulation.py`, GEE-Fits via `fit_h3_model` auf simulierten Datensätzen, ICC aus Pilot-GEE `dep_params`) — siehe `docs/gate_e_h3_power_simulation.md`, Eintrag auch in `docs/consistency_log.md`. Kernbefund: konfirmatorischer TextWorld/TLE-Test erreicht 80 % Power nur für Interaktionseffekte in der Größenordnung des vollen Haupteffekts (praktisch vollständige Signaldegradation bis Episodenende); für moderate Degradation (25–50 % Abschwächung) liegt die Power nur bei ~15–35 %. Checkbox bewusst offen gelassen (kein HART-Punkt, keine Design-Entscheidung getroffen) — Befund ist für den Nutzer zur Entscheidung, nicht bereits umgesetzt.
...
| E | Analyse-Rehearsal | **teilweise** (beide HART-Punkte abgeschlossen und am `2026-07-20` gegen aktuellen Code re-verifiziert; WEICH-Punkt H3-Power-Simulation durchgeführt, Checkbox bewusst offen — Nutzer-Entscheidung, keine Restarbeit) |
