...
- [x] **HART — Beide Manifeste final und im Freeze-Tag.** `difficulty_manifest.json` für TextWorld und ToH mit 50 Instanzen, `difficulty_tier`, `holdout` (5/45); nach diesem Punkt unveränderlich. Ein Smoke bestätigt, dass `holdout`/`difficulty_tier` im Episode-JSON ankommen (Gate B belegt den Schreibpfad bereits). Beide Manifeste inhaltlich final (TextWorld: `r5_i1_take+cook`, Seed 20260718; ToH: 4 Disks, `random_scramble`, Seed 271828), Mock-Smoke für beide bestanden. **Freeze-Tag gesetzt: `phase1-go` → Commit `07cf08b` (2026-07-22)** — nach Abschluss von Gate F, dem Repo-Housekeeping-Refactor (PR #26) und einer finalen Re-Verifikation aller eingefrorenen Configs auf echter RunPod-Hardware (Backend-Parity, C1/C2-QC-Probe, realer Phase-1-Slice, Resume-Correctness-Smoke — alle bestanden). *Evidenz: Manifeste im getaggten Commit; `docs/consistency_log.md`, Einträge 2026-07-21/22.*
...
| D | Schwierigkeitskalibrierung + Manifeste | **abgeschlossen** (beide Korridor-HART-Punkte + Freeze-Tag `phase1-go` → `07cf08b`) |
...
**GO erreicht: 2026-07-22, Freeze-Tag `phase1-go` → Commit `07cf08b`.** Alle Gates A–F abgeschlossen. Änderungen an diesem Dokument ab hier nur noch als datiertes Amendment unterhalb dieser Zeile.
