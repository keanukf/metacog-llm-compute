...
- [ ] **HART — Beide Manifeste final und im Freeze-Tag.** `difficulty_manifest.json` für TextWorld und ToH mit 50 Instanzen, `difficulty_tier`, `holdout` (5/45); nach diesem Punkt unveränderlich. Ein Smoke bestätigt, dass `holdout`/`difficulty_tier` im Episode-JSON ankommen (Gate B belegt den Schreibpfad bereits). **Stand 2026-07-21:** beide Manifeste existieren, sind committed und inhaltlich final (TextWorld: `r5_i1_take+cook`, Seed 20260718; ToH: 4 Disks, `random_scramble`, Seed 271828), Mock-Smoke für beide bestanden — inhaltlich nichts mehr zu tun. Noch offen: der eigentliche **Freeze-Tag** (git tag) selbst — laut diesem Dokument (Zeile 19/115) gibt es einen einzigen projektweiten Tag als finaler GO-Moment direkt vor dem ersten Phase-1-Block. Gate F ist seit 2026-07-21 komplett abgeschlossen, der Tag könnte damit technisch jetzt gesetzt werden — **bewusst nochmal vertagt** (Nutzerentscheidung 2026-07-21): erst nach dem geplanten Repo-Refactor + der finalen Re-Verifikation aller eingefrorenen Configs, damit der getaggte Commit wirklich der Endzustand ist, mit dem Phase 1 läuft, statt von der nachfolgenden Refactor-Arbeit sofort wieder überholt zu werden. *Evidenz: Manifeste im getaggten Commit (folgt nach Refactor + Re-Verifikation).*
...
- [x] **HART — Run-Hygiene auf dem Pod.** Modell vorab auf der ephemeren Container-Disk gecacht (bewusst **nicht** auf dem persistenten Network Volume — RunPods Template-Default zeigt `HF_HOME` fälschlich auf `/workspace/.cache`, `scripts/pod_runtime_env.sh` korrigiert das; Network Volume bleibt klein, nur Code + Results, Re-Download nach Container-Neustart ist einkalkuliert); Langfuse-Keys gesetzt oder Tracing bewusst aus (dokumentiert); Repo-Checkout selbst liegt unter `/workspace` (das ist der eigentliche Persistenz-Mechanismus, nicht die `RESULTS_DIR`-Env-Var, die von keinem Script gelesen wird); History-Guard aktiv; **Logprob-Sidecars:** `logprob_sidecar_mode: action_window`, **Reasoning-full** nur für Nicht-Holdout-Instanzen in `logprob_sidecar_full_instances` (z. B. 1–3, nicht Holdout mod-10); parallelisiertes Backend mit den in C-1 als batch-invariant verifizierten Einstellungen (N=32). **Preflight-Script:** `scripts/gate_f_run_hygiene_preflight.py` prüft alle fünf Punkte automatisiert, ohne GPU/Inferenz. **Real auf drei unabhängigen Pod-Sessions bestätigt (2026-07-20/21), jedes Mal 5/5 grün.** *Evidenz: `scripts/gate_f_run_hygiene_preflight.py`-Output, `docs/consistency_log.md`.*
- [x] **HART — C1/C2-Qualitätskontrolle unter den aktuellen, kalibrierten Korridoren.** Gate C-2 (Format-Compliance) lief auf einem kurzen Smoke, vor der Gate-D-Kalibrierung und vor dem `max_steps`-Fix (`fix/toh-per-instance-max-steps`) — die jetzt viel längeren Episoden (TW Cap 45, ToH bis 45 pro Instanz) sind seitdem nie mit echtem C1/C2-Reasoning durchlaufen worden. **Erster echter Lauf (2026-07-20) fand einen realen Bug:** C1 leakte den literalen String `'<think>'` als Aktion, wenn Reasoning nie schloss (siehe `docs/consistency_log.md`) — gefixt durch Vereinheitlichung von C1/C2 auf eine geteilte Reasoning-Engine (PR #25, gemergt). **Real-Recheck (2026-07-21) nach dem Fix, n=2/Zelle:** kein einziges `<think>`-Leak mehr; TextWorld/C1 und TextWorld/C2 zeigen nur noch legitime `cot_max_tokens=8192`-Trunkierungen (Modell braucht gelegentlich zu lange, sauber als `no_admissible_samples`/leere Aktion behandelt, kein Bug); ToH/C1 und ToH/C2 komplett sauber. Voting-Korrektheit erneut bestätigt (aufgezeichneter Sieger stimmt immer mit unabhängig neu berechnetem Mehrheitsvotum überein). *Evidenz: `scripts/gate_f_c1c2_quality_probe.py`-Output, `docs/consistency_log.md`.*
...
| F | Ops + Budget (5090, Batching) | **abgeschlossen** (Budget/Speicher/Top-up, Resume-Korrektheit (PR #23), Run-Hygiene (3× real bestätigt), C1/C2-QC (PR #25, real bestätigt); Block-Aufteilung bewusst gestrichen) |
