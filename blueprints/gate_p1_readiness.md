...
- [ ] **WEICH — H3-Power-Simulation.** §5.8 sieht eine simulationsbasierte Power-Prüfung für die Interaktion vor, geseedet mit Pilot-ICC und Entropieverteilung. Nach C-5 entweder durchführen oder die Limitation aktiv wählen und in §5.9 belassen. Vor Phase 1 entscheiden, welcher Pfad gilt. **Durchgeführt (2026-07-17), neu gelaufen mit korrigierter TextWorld-Längen-Annahme (2026-07-19):** echte Monte-Carlo-Power-Simulation (`scripts/h3_power_simulation.py`, GEE-Fits via `fit_h3_model` auf simulierten Datensätzen, ICC aus Pilot-GEE `dep_params`) — siehe `docs/gate_e_h3_power_simulation.md`, Einträge auch in `docs/consistency_log.md`. Kernbefund (aktuell, 15–40-Step-Korridor): konfirmatorischer TextWorld/TLE-Test erreicht 80 % Power nur für Interaktionseffekte in der Größenordnung des vollen Haupteffekts (praktisch vollständige Signaldegradation bis Episodenende, |β_int|≈0.228); für moderate Degradation (25–50 % Abschwächung) liegt die Power nur bei ~42–51 % (α=.025/.05) — besser als die ursprüngliche 8–15-Step-Annahme (~23–35 %), aber weiterhin klar unter 80 % für den plausibelsten Fall. Checkbox bewusst offen gelassen (kein HART-Punkt, keine Design-Entscheidung getroffen) — Befund ist für den Nutzer zur Entscheidung (§5.9-Ergänzung im Thesis-Repo), nicht bereits umgesetzt.
...
| F | Ops + Budget (5090, Batching) | **teilweise** (Budget/Speicher @ `9f8dafd`; Resume-Korrektheit abgeschlossen (PR #23); Run-Hygiene + Budget-Top-up offen) |
