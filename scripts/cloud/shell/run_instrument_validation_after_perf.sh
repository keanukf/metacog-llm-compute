...
source "$REPO_ROOT/scripts/cloud/shell/activate_pod_env.sh" 2>/dev/null || true
...
$PYTHON scripts/instrument_validation/apply_production_n.py "$IV/throughput_sweep_post_perf.json"
...
$PYTHON scripts/pilot_analysis/audit_pilot_signals.py "$PROBE" --json | tee "$PROBE/audit_signals.json"
