...
def _print_progress(
    *,
    ...
    if rolling:
        ...
        print(msg)
    insts = [r.get("instance") for r in rolling if r.get("domain") == domain and r.get("strategy") == stage_or_strategy]
    if insts:
        print(f"  Domain: {domain} | Strategy: {stage_or_strategy} | Instance range: {min(insts)}–{max(insts)}")
    else:
        print(f"  Domain: {domain} | Strategy: {stage_or_strategy}")
...
def main() -> None:
    ...
    progress_every = int(args.progress_every) if int(args.progress_every) > 0 else int(phase2.get("progress_every_episodes", 10))
