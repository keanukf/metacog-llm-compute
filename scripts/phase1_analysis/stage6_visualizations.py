...
H3 signal x position marginal-effect curves per domain/signal (with an empirical binned-data
overlay from the canonical dataset, so the plot doubles as a linearity-in-logit model-fit check,
not just a visualization of what the model implies). Otherwise a plotting pass over already-
computed stage output -- no new statistics -- so it's safe to re-run anytime after Stages 0, 2
and 4.
